# -*- coding: utf-8 -*-
# """
# src/sandbox_plaid/get_balance.py
...
logger.info(msg="=== Running `src/sandbox_plaid/get_balance.py`===")
...
JSON_FILE_PATH: str | None = (
    os.path.join(DATA_DIR, "balance.json") if DATA_DIR else None
)
CSV_FILE_PATH: str | None = os.path.join(DATA_DIR, "balance.csv") if DATA_DIR else None
...
def run_workflow() -> None:
    ...
    for institution_id in INSTITUTION_IDS_TO_PROCESS:
        ...
        # Make the POST Request to get balance
        logger.info(
            msg=f"Access token for {institution_id} is ready. Proceeding to fetch balance..."
        )

        # Step 3: Get Balance
        wait_time: int = cfgs.get("WAIT_TIME", 5)
        logger.info(
            msg=f"Waiting for {wait_time} seconds before fetching balance for {institution_id}..."
        )
        ...
        if JSON_FILE_PATH:
            ...
            if CSV_FILE_PATH:
                convert_json_to_csv(
                    ...
                    key="accounts",  # or "accounts" based on your need
    ...
    logger.info(
        msg=f"=== Product: 'accounts'/{INITIAL_PRODUCTS[0]} Workflow Completed ==="
    )
